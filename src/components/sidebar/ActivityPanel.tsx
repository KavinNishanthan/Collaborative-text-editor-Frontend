  ...
  return (
    <div className="flex flex-col">
      ...
      {!loading && logs.map((log) => (
        <div key={log.logId} className="flex items-start gap-3 px-4 py-3 border-b border-black/[0.04]">
          <span className="text-base flex-shrink-0 mt-0.5">{actionIcon[log.action] || '📝'}</span>
          <div className="flex-1 min-w-0">
            <p className="text-sm text-[#334155]">
              <span className="font-semibold text-[#0F172A]">{log.user?.name || 'Someone'}</span>
              {' '}<span className="text-[#64748B]">{log.action}</span>
            </p>
            <p className="text-[11px] text-[#94A3B8] mt-0.5">{formatDate(log.timestamp)}</p>
          </div>
        </div>
      ))}
    </div>
