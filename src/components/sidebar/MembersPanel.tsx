          ...
          <form onSubmit={handleInvite} className='flex flex-col gap-2'>
            ...
            <div className='flex gap-2'>
              ...
              <button
                type='submit'
                disabled={inviting}
                className='flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium bg-[#4F46E5] text-white hover:bg-[#4338CA] disabled:opacity-50 transition-all shrink-0'>
                {inviting ? (
                  <span className='w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin-custom' />
                ) : (
                  <UserPlus size={14} />
                )}
                Invite
        ...
        {!loading &&
          members.map((m) => (
            <div key={m.memberId} className='flex items-center gap-3 px-4 py-3'>
              <div className='w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white bg-linear-to-br from-[#4F46E5] to-[#06B6D4] shrink-0'>
              ...
              <div className='flex items-center gap-1.5'>
                {isOwner && m.role !== "owner" ? (
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide ${roleColor[m.role] || roleColor.viewer}`}>
                    {m.role}
                  </span>
                )}
              </div>
