    ...
    <div className='flex flex-col h-full bg-white'>
      ...
      {loading && (
        <div className='flex justify-center py-12'>
          <span className='w-5 h-5 border-2 border-black/8 border-t-[#4F46E5] rounded-full animate-spin-custom' />
        </div>
      )}
      ...
      {!loading &&
        groups.map(([dateLabel, entries]) => (
          <div key={dateLabel}>
            {/* Date group header */}
            <div className='px-4 py-2 bg-[#F8FAFC] border-y border-black/4 sticky top-0 z-10'>
            ...
            {entries.map((h) => {
              ...
              return (
                <div
                  ...
                  className={`
                  group flex items-start gap-3 px-4 py-3.5 border-b border-black/4 cursor-pointer
                ...
                `}>
                  {/* User avatar */}
                  <div
                    className='w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold text-white shrink-0 mt-0.5 shadow-sm'
