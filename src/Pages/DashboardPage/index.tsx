...
export default function Dashboard() {
  ...
  return (
    <div className='h-screen flex flex-col bg-[#F8FAFC] text-[#0F172A] overflow-hidden'>
      <header className='h-14 border-b border-black/6 bg-white flex items-center gap-2 px-3 shrink-0 z-40'>
        <button
        ...
        {historyPreview && (
          <div className='flex items-center gap-1.5 px-3 py-1 rounded-full bg-[rgba(245,158,11,0.1)] text-[#D97706] text-xs font-medium shrink-0'>
            <History size={12} />
            Preview Mode
          </div>
        )}
        ...
        <div className='flex items-center gap-1 shrink-0'>
            ...
            <>
              {[
                ...
                <button
                  ...
                  title={label}
                  className={`p-2 rounded-xl transition-all ${panel === key ? "bg-[rgba(79,70,229,0.08)] text-[#4F46E5]" : "text-[#94A3B8] hover:text-[#0F172A] hover:bg-black/4"}`}>
                  <Icon size={17} />
                </button>
              ))}
            </>
          ...
          {activeDoc && <div className='w-px h-6 bg-black/6 mx-1' />}
          ...
          <div className='relative'>
            <button
              ...
              className='p-2 rounded-xl text-[#94A3B8] hover:text-[#0F172A] hover:bg-black/4 transition-all relative'
              title='Notifications'>
              <Bell size={17} />
            ...
            {inboxOpen && (
              <>
                ...
                />
                <div className='absolute right-0 top-full mt-2 w-80 max-w-[90vw] bg-white border border-black/8 rounded-2xl shadow-[0_8px_48px_rgba(0,0,0,0.12)] z-50 overflow-hidden animate-slide-up'>
                    ...
                    {invitations.length === 0 ? (
                      ...
                      invitations.map((inv) => (
                        <div
                          ...
                          className='px-4 py-3 border-b border-black/4 last:border-0 hover:bg-[#F8FAFC] transition-colors'>
          ...
          <div className='hidden sm:flex items-center gap-2 px-2.5 py-1 rounded-xl bg-[#F8FAFC] border border-black/6'>
            <div className='w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white bg-linear-to-br from-[#4F46E5] to-[#06B6D4]'>
              {getInitials(user?.name || "U")}
            </div>
            <span className='text-xs font-medium text-[#0F172A] truncate max-w-20'>
              {user?.name}
            </span>
          </div>
          <button
      ...
      <div className='flex flex-1 overflow-hidden'>
        ...
        <aside
          className={`${sidebarOpen ? "w-72 translate-x-0" : "w-0 -translate-x-full md:translate-x-0"} fixed md:relative z-30 md:z-auto top-0 left-0 h-full md:h-auto shrink-0 border-r border-black/6 bg-white flex flex-col transition-all duration-200 overflow-hidden`}>
          <div className='p-3 flex flex-col gap-2 shrink-0'>
            ...
            <div className='relative'>
              ...
              <input
                ...
                className='w-full pl-8 pr-3 py-2 bg-[#F8FAFC] border border-black/6 rounded-xl text-xs text-[#0F172A] placeholder-[#94A3B8] outline-none focus:border-[#4F46E5] transition-all'
              />
            </div>
          </div>
            ...
            {!docsLoading &&
              filtered.map((doc) => (
                <div
                  ...
                  <FileText
                    size={16}
                    className={`mt-0.5 shrink-0 ${activeDocId === doc.documentId ? "text-[#4F46E5]" : "text-[#94A3B8]"}`}
                  />
                  ...
                  {doc.role === "owner" && (
                    <button
                      ...
                      className='opacity-0 group-hover:opacity-100 p-1 rounded-lg text-[#94A3B8] hover:text-[#EF4444] hover:bg-[rgba(239,68,68,0.06)] transition-all shrink-0 mt-0.5'>
                      <Trash2 size={13} />
                    </button>
                  )}
                </div>
          ...
          <div className='flex-1 overflow-y-auto editor-canvas'>
            {!activeDocId && !docLoading && (
              <div className='flex flex-col items-center justify-center h-full gap-5 text-center px-6'>
                <div className='w-20 h-20 rounded-3xl bg-linear-to-br from-[rgba(79,70,229,0.08)] to-[rgba(6,182,212,0.06)] border border-[rgba(79,70,229,0.1)] flex items-center justify-center text-[#4F46E5]'>
                  <FileText size={36} />
                </div>
                <div>
                  <h2 className='text-2xl font-bold text-[#0F172A] mb-1.5'>
                    Select a document
                  </h2>
                ...
                <button
                  onClick={handleCreate}
                  disabled={creating}
                  className='inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm bg-[#4F46E5] text-white shadow-[0_2px_12px_rgba(79,70,229,0.25)] hover:bg-[#4338CA] disabled:opacity-50 transition-all'>
                  {creating ? (
                    <span className='w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin-custom' />
                  ) : (
                    <Plus size={15} />
          ...
          <>
            ...
            <aside className='fixed right-0 top-0 h-full w-80 max-w-[85vw] z-30 lg:relative lg:z-auto shrink-0 border-l border-black/6 bg-white flex flex-col overflow-hidden shadow-[-4px_0_24px_rgba(0,0,0,0.08)] lg:shadow-none'>
              <div className='flex items-center justify-between px-4 py-3 border-b border-black/6 shrink-0'>
                <h2 className='font-semibold text-sm capitalize text-[#0F172A]'>
                  {panel}
                </h2>
                <button
                  onClick={() => {
                  ...
                  className='p-1 rounded-lg text-[#94A3B8] hover:text-[#0F172A] hover:bg-black/4 transition-all'>
                  <X size={16} />
                </button>
              </div>
      ...
      {deleteConfirm && (
        <div className='fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/20 backdrop-blur-sm animate-fade-in'>
          <div className='bg-white border border-black/8 rounded-2xl p-7 max-w-sm w-full shadow-[0_8px_48px_rgba(0,0,0,0.1)] animate-slide-up'>
            <div className='w-12 h-12 rounded-xl bg-[rgba(239,68,68,0.06)] flex items-center justify-center text-[#EF4444] mb-4'>
              <Trash2 size={22} />
            </div>
            <h3 className='font-bold text-lg mb-2 text-[#0F172A]'>
              Delete document?
            </h3>
            ...
            <div className='flex gap-3'>
              <button
                ...
                className='flex-1 py-2.5 rounded-xl text-sm font-medium bg-[#F8FAFC] border border-black/8 hover:bg-[#F1F5F9] text-[#0F172A] transition-all'>
                Cancel
              </button>
              <button
                onClick={() => handleDelete(deleteConfirm)}
                className='flex-1 py-2.5 rounded-xl text-sm font-medium bg-[rgba(239,68,68,0.08)] text-[#EF4444] border border-[rgba(239,68,68,0.15)] hover:bg-[rgba(239,68,68,0.14)] transition-all'>
                Delete
              </button>
            </div>
          </div>
