...
// Custom CollaborationCursor extension that uses @tiptap/y-tiptap's
// yCursorPlugin (matching the ySyncPlugin used by @tiptap/extension-collaboration v3).
// The official @tiptap/extension-collaboration-cursor v2 uses y-prosemirror's
// cursor plugin which has a DIFFERENT ySyncPluginKey, causing crashes.
      ...
      yCursorPlugin(
        provider.awareness,
        {
          awarenessStateFilter: (
            ...
            if (localUserId && otherState?.user?.userId === localUserId) return false;
            return true;
          },
          ...
          cursorBuilder: (awarenessUser: Record<string, any>) => {
            ...
            cursor.setAttribute('style', `border-color: ${awarenessUser.color || '#6c63ff'}`);

            const label = document.createElement('div');
            label.classList.add('collaboration-cursor__label');
            label.setAttribute('style', `background-color: ${awarenessUser.color || '#6c63ff'}`);
